...
def get_mac_address():
    ...
    for s in c.Win32_NetworkAdapter():
        if s.MACAddress is not None and s.PhysicalAdapter:
            macAddrs.append(s.MACAddress)
    sorted_mac = sorted(macAddrs)
    mac = ""
    for m in sorted_mac:  # 物理地址拼起来
        mac += m + '-'  # 遍历网络接口信息，获取 MAC 地址
    return mac[:-1]
...
class CreateVideo:
    ...
    def thread_func(self, book, path, db, config, scene_tag, sd_config):
        ...
        fail_info = ''
        try:
            self.main(book, path, db, config, scene_tag, sd_config)
    ...
    def main(self, book, path, db, config, scene_tag, sd_config):
        ...
        if count > 0 and count == len(text_list):
            ...
            object_list = self.create_prompt_words(text_list, scene_tag)
            print(object_list)
            data_list = []
    ...
    def create_prompt_words(self, text_list, tags=None):
        """
        生成提示词
        :return:
        """
